...
| B-AX-2 (fixed) | the editor's axis boxes | Every keystroke was applied, so typing 400000 applied 4, 40, 400 ... and each clamped the quadrants into that range. Fixed: the boxes (`CommittedNumber`) apply a number on Enter or on leaving the box. What a deliberate narrowing still does is B-AX-4 | - |
