...
- **1,123 unit tests and 32 integration tests pass**, plus 11 doctests.
- **27 known-bug tests (25 bugs) are pinned as failing tests** (`#[ignore]`d
  ...
  B-AX-3, B-AX-4, B-HIER-2, B-META-1, B-SCALE-1 and B-OMIQ-2 have been
  fixed.
...
| B-CNT-1 | `gate_filtering::filter_events_to_mask` vs `gate_stats` (`EventIndex`) | The filter admits strictly inside a rectangle, the index counts the edge: the percentage on a gate counts events the population drawn under it does not hold (240 vs 246 in the test, the six edge events) | Medium - whole-number scatter values meet round edges. Checked: `flow_gates` counts a rectangle's edges as inside everywhere (`rectangle.rs`, `types.rs`, `batch_filtering.rs`), and so does the on-screen percentage built on it; only clingate's own polars filter (`gate_filtering.rs`) excludes them. On float data an event lands exactly on an edge almost never, which is why whole-hierarchy percentages matched Omiq. Proposed fix: make the filter inclusive (`gt_eq`/`lt_eq`), so the population under a gate is the one its percentage counts |
